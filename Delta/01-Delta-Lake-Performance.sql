...
-- Set catalog and ddatabase to use
USE CATALOG hive_metastore;
USE DATABASE retail_chiayui_lee;

-- Create Delta table for this exercise
CREATE TABLE IF NOT EXISTS user_delta_optimize
AS
...
-- MAGIC %python
-- MAGIC # Inspect files for the user_delta_optimize table
-- MAGIC display(
-- MAGIC   dbutils.fs.ls('dbfs:/Users/chiayui.lee@databricks.com/demos/retail/tables/user_delta_optimize')
-- MAGIC )

-- COMMAND ----------

-- let's compact our table. Note that the engine decided to compact 8 files into 1 ("numFilesAdded": 1, "numFilesRemoved": 8)
OPTIMIZE user_delta_optimize 
...
ALTER TABLE user_delta_optimize SET TBLPROPERTIES (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true);
...
-- MAGIC 
-- MAGIC *Note: We recommend to stay below 4 ZORDER columns for better query performance.*

-- COMMAND ----------

OPTIMIZE user_delta_optimize ZORDER BY (id, firstname);

-- our next queries using a filter on id or firstname will be much faster
SELECT * FROM user_delta_optimize where id = 4 or firstname = 'Quentin';
...
VACUUM user_delta_optimize RETAIN 0 HOURS DRY RUN
...
VACUUM user_delta_optimize RETAIN 0 HOURS
...
-- MAGIC   age_group,
-- MAGIC   creation_date
-- MAGIC FROM user_delta_optimize;
...
-- MAGIC # Inspect files for the user_delta_optimize table
-- MAGIC display(
-- MAGIC   dbutils.fs.ls('dbfs:/Users/chiayui.lee@databricks.com/demos/retail/tables/user_delta_partition')
-- MAGIC )
